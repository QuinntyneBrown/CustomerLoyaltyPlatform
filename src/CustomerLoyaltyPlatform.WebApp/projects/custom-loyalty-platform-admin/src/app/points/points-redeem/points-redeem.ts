...
interface PointsRedeemViewModel {
  loading: boolean;
  submitting: boolean;
  error: string | null;
  tenantId: string;
...
@Component({
  ...
  templateUrl: './points-redeem.html',
  styleUrl: './points-redeem.scss'
})
export class PointsRedeem {
  ...
  vm$: Observable<PointsRedeemViewModel> = combineLatest([
    this.loading$,
    this.submitting$,
    this.error$,
    this.tenantId$,
  ...
  submit(): void {
    ...
    this.pointsService.redeemPoints({
      ...
      next: () => {
        ...
        this.snackBar.open('Points redeemed successfully', 'Close', { duration: 3000 });
        this.router.navigate(['/tenants', tenantId, 'members', formValue.memberId, 'points']);
      },
      error: (err) => {
        this.submitting$.next(false);
        this.error$.next(err.error?.message || err.message || 'Failed to redeem points');
      }
    });
  }
